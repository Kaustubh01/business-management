import 'package:gsheets/gsheets.dart';

const credentials = r'''
{
  "type": "service_account",
  "project_id": "flutter-sheets-437409",
  "private_key_id": "308510fd3ea9d0c8f7998652c4cc5c01ea81174d",
  "private_key": "-----BEGIN PRIVATE KEY-----\\nGEWPJp81ywwKvqG4/C+iFU90\n-----END PRIVATE KEY-----\n",
  "client_email": "flutter-sheets@flutter-sheets-437409.iam.gserviceaccount.com",
  "client_id": "107926352547901415634",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/flutter-sheets%40flutter-sheets-437409.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}
''';


// Class to encapsulate the spreadsheet service
class InventorySpreadsheetService {
  final String _spreadsheetId = '1JRqJMZt-PO8gtUytNW2wTqp-wc8olicuNt04QM8oPlU'; // Replace with your actual Spreadsheet ID


  Future<GSheets> _initializeGsheets() async {
    final gsheets = GSheets(credentials);
    return gsheets;
  }

  // Function to create a new worksheet for a business ID if it does not exist
  // Function to create a new worksheet for a business ID if it does not exist
  Future<void> createInventorySheetIfNotExists(String businessId) async {
  final gsheets = await _initializeGsheets();
  final spreadsheet = await gsheets.spreadsheet(_spreadsheetId);

  // Check if the worksheet already exists
  final existingSheet = spreadsheet.worksheetByTitle(businessId);
  
  if (existingSheet == null) {
    // Create a new worksheet with the business ID
    final sheet = await spreadsheet.addWorksheet(businessId);

    // Add headers to the new worksheet
    final headers = ['Product ID', 'Product Name', 'Brand', 'Unit Price (USD)', 'Quantity'];
    await sheet.values.insertRow(1, headers);

    print('Worksheet for business ID "$businessId" created successfully with headers.');
  } else {
    print('Worksheet for business ID "$businessId" already exists.');
  }
}


  // Function to fetch the product inventory data from the spreadsheet
  Future<List<Map<String, String>>> getInventoryData(String sheetTitle) async {
    final gsheets = await _initializeGsheets();
    final spreadsheet = await gsheets.spreadsheet(_spreadsheetId);

    // Access the worksheet by its title (Sheet1)
    final sheet = spreadsheet.worksheetByTitle(sheetTitle);

    if (sheet != null) {
      // Fetch all rows
      final rows = await sheet.values.allRows();

      // Convert rows into a list of maps, where each map represents a product
      final List<Map<String, String>> inventoryData = [];
      
      // The first row contains column headers
      final headers = rows[0];

      // Iterate over the rows, starting from the second row (index 1)
      for (int i = 1; i < rows.length; i++) {
        final Map<String, String> product = {};
        for (int j = 0; j < headers.length; j++) {
          product[headers[j]] = rows[i][j];
        }
        inventoryData.add(product);
      }
      
      return inventoryData;
    } else {
      throw Exception('Worksheet not found');
    }
  }

  // Function to get the maximum product ID from the inventory
  Future<int> getMaxProductId(String sheetTitle) async {
    final gsheets = await _initializeGsheets();
    final spreadsheet = await gsheets.spreadsheet(_spreadsheetId);

    // Access the worksheet by its title
    final sheet = spreadsheet.worksheetByTitle(sheetTitle);

    if (sheet != null) {
      // Fetch all rows
      final rows = await sheet.values.allRows();

      // Check if there are any rows
      if (rows.length < 2) return 0; // No products, start from 0

      // Assuming the first column contains the product ID
      final List<int> ids = rows
          .skip(1) // Skip header row
          .map((row) => int.tryParse(row[0]) ?? 0) // Parse IDs, using 0 if parsing fails
          .toList();

      // Filter out any duplicate IDs and return the max value
      return ids.isNotEmpty ? ids.reduce((a, b) => a > b ? a : b) : 0; // Return max ID or 0 if empty
    } else {
      throw Exception('Worksheet not found');
    }
  }

  // Function to add a new inventory item
  Future<void> addInventoryItem(String sheetTitle, Map<String, String> itemData) async {
    final gsheets = await _initializeGsheets();
    final spreadsheet = await gsheets.spreadsheet(_spreadsheetId);
    
    // Access the worksheet by its title
    final sheet = spreadsheet.worksheetByTitle(sheetTitle);

    if (sheet != null) {
      // Get the current max ID and increment it for the new item
      int maxId = await getMaxProductId(sheetTitle);
      int newId = maxId + 1; // Increment ID for the new item
      
      // Prepend the new ID to the item data
      itemData['Product ID'] = newId.toString();
      
      // Get headers to ensure the new row matches the existing structure
      final headers = await sheet.values.row(1);

      // Create a list to hold the new row data
      final List<String> newRow = List.filled(headers.length, '');

      // Fill in newRow based on headers and the itemData map
      for (int i = 0; i < headers.length; i++) {
        newRow[i] = itemData[headers[i]] ?? '';
      }

      // Append the new item data as a new row in the correct order
      await sheet.values.appendRow(newRow);
      print('New inventory item added successfully with ID: $newId');
    } else {
      throw Exception('Worksheet not found');
    }
  }

  // Function to update an existing inventory item
  Future<void> updateInventoryItem(String sheetTitle, String itemId, Map<String, String> updatedData) async {
    final gsheets = await _initializeGsheets();
    final spreadsheet = await gsheets.spreadsheet(_spreadsheetId);
    
    // Access the worksheet by its title
    final sheet = spreadsheet.worksheetByTitle(sheetTitle);

    if (sheet != null) {
      // Fetch all rows
      final rows = await sheet.values.allRows();

      // Find the index of the row with the given item ID
      int itemIndex = -1;
      for (int i = 1; i < rows.length; i++) {
        if (rows[i][0] == itemId) { // Assuming the first column contains the item ID
          itemIndex = i;
          break;
        }
      }

      if (itemIndex != -1) {
        // Get headers to ensure the update matches the existing structure
        final headers = await sheet.values.row(1);

        // Create a list for updated row data
        final List<String> updatedRow = List.filled(headers.length, '');

        // Populate updatedRow based on headers and updatedData map
        for (int i = 0; i < headers.length; i++) {
          updatedRow[i] = updatedData[headers[i]] ?? rows[itemIndex][i]; // Keep existing data if no update provided
        }

        // Update the row with new data
        await sheet.values.insertRow(itemIndex + 1, updatedRow);
        print('Inventory item with ID "$itemId" updated successfully.');
      } else {
        print('Item with ID "$itemId" not found.');
      }
    } else {
      throw Exception('Worksheet not found');
    }
  }
}
